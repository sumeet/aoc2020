...
def transform1(oldmap):
    newmap = oldmap.copy()
    for point, status in oldmap.items():
        if status == "L" and num_occupied_adjacents(oldmap, point) == 0:
            newmap[point] = "#"
        if status == "#" and num_occupied_adjacents(oldmap, point) >= 4:
            newmap[point] = "L"
    return newmap


def num_occupied_adjacents(map, point):
    return sum(1 for p in adjacents(map, point) if p == "#")


def adjacents(map, point):
    def add(p1, p2):
        x1, y1 = p1
        x2, y2 = p2
        return (x1 + x2), (y1 + y2)
...
def part1():
    ...
    while True:
        newseatmap = transform1(seatmap)
        if newseatmap == seatmap:
            printmap(seatmap)
            print(sum(1 for seat in seatmap.values() if seat == "#"))
            break
        else:
            seatmap = newseatmap


def num_occupied_adjacents2(map, point):
    def add(p1, p2):
        x1, y1 = p1
        x2, y2 = p2
        return (x1 + x2), (y1 + y2)
    ...
    for direction in all_directions:
        searchpoint = point
        done_with_direction = False
        while not done_with_direction:
            searchpoint = add(searchpoint, direction)
...
def transform2(oldmap):
    newmap = oldmap.copy()
    for point, status in oldmap.items():
        if status == "L" and num_occupied_adjacents2(oldmap, point) == 0:
            newmap[point] = "#"
        if status == "#" and num_occupied_adjacents2(oldmap, point) >= 5:
            newmap[point] = "L"
    return newmap
