...
def parse_input
  ...
  rules = rule_lines.map { |line| line.split(": ") }
    .sort_by { |(key, _)| key.to_i }
    .map { |(_, rule_only)| parse_rule(rule_only) }
  {rules, messages}
end

def parse_rule(rule_only) : Rule
  if rule_only.includes?('|')
    left, right = rule_only.split(" | ")
    Or.new(parse_rule(left), parse_rule(right))
  elsif rule_only.includes?(' ')
    rule_only.split(' ').map { |rule| parse_rule(rule) }
  elsif rule_only.includes?('"')
    rule_only[1] # for "b" pull out b
  else
    rule_only.to_i16
  end
end
...
class Or
  ...
  def initialize(left_rule, right_rule)
    @left = left_rule
    @right = right_rule
  end
end

alias MatchRuleRef = Int16
alias MatchChar = Char
alias Series = Array(Rule)
alias Rule = MatchRuleRef | MatchChar | Or | Series

rules, messages = parse_input

# fix rules for part b
rules[8] = parse_rule("42 | 42 8")
rules[11] = parse_rule("42 31 | 42 11 31")

# try getting rid of references
def contains_reference?(rule)
  case rule
  when MatchRuleRef
    true
  when MatchChar
    false
  when Series
    rule.any? { |inner_rule| contains_reference?(inner_rule) }
  when Or
    contains_reference?(rule.left) || contains_reference?(rule.right)
  else # it's already exhaustive, shouldn't need this
    false
  end
end

def replace_references(rule, all_rules) : Rule
  case rule
  when MatchRuleRef
    all_rules[rule]
  when MatchChar
    rule
  when Series
    rule.map do |inner_rule|
      replace_references(inner_rule, all_rules).as(Rule)
    end
  when Or
    Or.new(replace_references(rule.left, all_rules),
      replace_references(rule.right, all_rules))
  else # it's already exhaustive, shouldn't need this
    rule
  end
end

while contains_reference?(rules[0])
  rules.map! { |rule| replace_references(rule, rules) }
end

alias Rest = String

def matches?(rule, message) : Bool | Rest
  return true if message.empty?

  message = case rule
            when MatchChar
              return false if rule != message[0]
              message[1..]
            when Series
              rule.each do |inner_rule|
                inner_match = matches?(inner_rule, message)
                case inner_match
                when Bool
                  return inner_match
                else
                  message = inner_match.as(Rest)
                end
              end
              message
              # rule.map do |inner_rule|
              ...
              # end
            when Or
              or_result = matches?(rule.left, message) || matches?(rule.right, message)
              case or_result
              when Bool
                return or_result
              else
                message = or_result.as(Rest)
              end
            else
              puts "error: somehow got to this else"
              false
            end
